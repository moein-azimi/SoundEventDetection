...
with open('feature_data_resnet50-b.pkl', 'rb') as f:
...
for emb, label in dataset_Xception[:]:
    if label.split('-')[0] != 'laugh': 
        if label.split('-')[0] != 'sneeze': 
            print(label.split('-')[0])
...
class_names = ['BabyCry', 'Breathing', 'Cough', 'Dishes', 'DoorClapping', 'DoorOpening', 
               ...
               'HairDryer', 'HandsClapping', 'Keyboard', 'Keys', 'MaleScream', 
               'Paper', 'Speech', 'Water', 'Yawn']
